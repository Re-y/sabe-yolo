...
class MEEA(nn.Module):
    def __init__(self, in_dim, hidden_dim, width, norm, act):
        ...
        self.pool = nn.AvgPool2d(3, stride=1, padding=1)  # 尺寸未变

        # self.cond_pool=ConditionalPooling(in_channels=in_dim)
        ...
        self.weight_level_4 = nn.Conv2d(hidden_dim, 4, 1)
        self.weight_level_2 = nn.Conv2d(width*4, 5, 1)
    ...
    def forward(self, x):
        ...
        for i in range(self.width - 1):
            ...
            out_list.append(self.edge_enhance[i](mid))

        for i in range(self.width):
            weight_list.append(self.weight_level_4(out_list[i]))

        weight_jia=torch.cat(weight_list, dim=1)
        level_weight_jia = self.weight_level_2(weight_jia)
        level_weight_jia = F.softmax(level_weight_jia, dim=1)
        # width = 4
        fused_jia = out_list[0] * level_weight_jia[:, 0:1, :, :] + \
                    out_list[1] * level_weight_jia[:, 1:2, :, :] + \
                    out_list[2] * level_weight_jia[:, 2:3, :, :] + \
                    out_list[3] * level_weight_jia[:, 3:4, :, :]


        out = self.out_conv(fused_jia)
...
class MEEMAM(nn.Module):
    def __init__(self, dim_in, norm_in=True, has_skip=True, exp_ratio=1.0, norm_layer='bn_2d',
        ...
        self.meea = MEEA(dim_in, dim_in//2, 4, nn.BatchNorm2d, nn.ReLU)
